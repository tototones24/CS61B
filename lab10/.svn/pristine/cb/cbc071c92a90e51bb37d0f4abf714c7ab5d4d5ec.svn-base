...
import static bugs.bug2.db61b.Tokenizer.*;
...
class CommandInterpreter {
    ...
    void loadStatement() {
        _input.next("load");
        String nme = name();
        Table tbl = Table.readTable(nme);
        _input.next(";");
        _database.put(nme, tbl);
        System.out.println("Loaded " + nme + ".db");
    }
    ...
    void printStatement() {
        ...
        _input.next("print");
        String nme = name();
        Table tbl = _database.get(nme);
        _input.next(";");
        System.out.println("Contents of " + nme + ":");
        tbl.print();
    }
    ...
    void selectStatement() {
        ...
        if (_input.nextIs("select")) {
            Table tbl = selectClause();
            System.out.println("Search Results:");
            tbl.print();
        } else {
            throw error("doesn't start with select");
        }
    ...
    Table selectClause() {
        _input.next("select");
        List<String> clmNames = new ArrayList<String>();
        while (!_input.nextIs("from")) { //parses all the columns.

            String clmName = columnName();
            clmNames.add(clmName);

            if (_input.nextIs(",")) {
                _input.next(","); // change to _input.nextIf(","); ???
            }

        }
        _input.next("from");

        /** see if there's 1 or 2 tables */
        boolean tbl2Exist = false;
        Table tbl = tableName(); //load the table
        Table tbl2 = null;
        if (_input.nextIf(",")) {
            tbl2 = tableName();
            tbl2Exist = true;
        ...
        /** parse the condition array */
        List<Condition> conds= new ArrayList<Condition>();
        if (_input.nextIf("where")) {
            if (!tbl2Exist) {
                conds.add(condition(tbl));
            } else if (tbl2Exist) {
                conds.add(condition(tbl, tbl2));
        ...
        //     if (tbl.findColumn(clmNames.get(i)) == -1) {
        //         throw error("invalid Column Name");
        //     }
        // }

        Table newTbl = tbl.select(clmNames, conds);
    ...
    Condition condition(Table... tables) {
        ...
        String lit = literal();
        Condition retValue = new Condition(new Column(colName, tables), rel, lit);

        return retValue;        // REPLACE WITH SOLUTION
    }
