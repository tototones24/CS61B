...
class Column {
    ...
    Column(String name, Table... tables) {
        ...
        for (_table = 0; _table < tables.length; _table += 1) {
            ...
            if (_column != -1) {
                return; // _table is now set to the index of the first table
                        // that has NAME in one of its columns. // same entities
            }
        }
    ...
    /** Return my index in the table I first appear in. */
    int getIndex() {
        return _column;
    }
