...
import static bugs.bug1.db61b.Tokenizer.*;
...
class CommandInterpreter {
    ...
    void loadStatement() {
        /* FILL THIS IN */
        _input.next("load");
        // debugging cases for statements like "load <nothing>"?
        Table toBeLoaded;
        String fileName = _input.next(Tokenizer.IDENTIFIER);
        toBeLoaded = Table.readTable(fileName);
        _database.put(fileName, toBeLoaded);
        _input.next(";");
        System.out.printf("Loaded %s.db%n", fileName);
    ...
    void storeStatement() {
        ...
        Table table = tableName();
        /* FILL THIS IN */
        table.writeTable(table.getName());
    ...
    void printStatement() {
        // FILL THIS IN
        _input.next("print");
        String tableName = _input.next(Tokenizer.IDENTIFIER);
    ...
    Table tableDefinition() {
        Table table;
        if (_input.nextIs("(")) { // maybe (_input.peek()).startsWith("(")?
    ...
    Table selectClause() {
        ...
        // result now has these columns. still needa add rows
        Table result = new Table(cols.toArray(new String[cols.size()]));
        _input.next("from");
        ArrayList<Table> tables = new ArrayList<>();
        String tableName = name();
        Table firstTable = _database.get(tableName);

        while (_input.nextIs(",")) {
            _input.next(",");
            tableName = name();
            Table nextTable = _database.get(tableName);
            tables.add(nextTable);
        }
        Table[] tablesArr = tables.toArray(new Table[tables.size()]);
        ArrayList<Condition> conds = null;
        if (_input.nextIs("where")) {
            _input.next("where");
            conds = conditionClause(tablesArr);
        }
        if (tablesArr.length == 1) {
            result = tablesArr[0].select(cols, conds);
        } else if (tablesArr.length == 2) {
            result = tablesArr[0].select(tablesArr[1], cols, conds);
        }
        _database.put(tableName, result);
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        return conds;        // REPLACE WITH SOLUTION
    }
    ...
    Condition condition(Table... tables) {
        ...
        Column col1 = new Column(colName1, tables);
        String op = _input.next(Tokenizer.RELATION);
        Condition cond;
        if ((_input.peek()).startsWith("'")) {
            String val = literal();
            cond = new Condition(col1, op, val);
        } else {
            String colName2 = name();
            Column col2 = new Column(colName2, tables);
            cond = new Condition(col1, op, col2);
        }
        return cond;        // REPLACE WITH SOLUTION
    }
