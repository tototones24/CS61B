...
class Game extends Observable {
        ...
        while (_exit < 0) {
            if (_playing) {
                try {
                    ...
                    reportError(validMove.getMessage());
                }
    ...
    private void setSpots(int r, int c, int spots, String color) {
        ...
        if (spots <= _board.neighbors(r, c)) {
            ...
            reportError("You are setting more spots, %d, than neighbors, %d.",
                spots, amountOfNeighs);
        }
    }
    ...
    private void readExecuteCommand() {
        if (!_inp.hasNextInt()) {
            ...
            } catch (InputMismatchException invalidSyntax) {
                reportError("syntax error in " + nextCommand);
            } catch (GameException badcommand) {
                reportError("Bad Command " + nextCommand);
            }
            _inp.nextLine();
            ...
            try {
                ...
                _inp.nextLine();
            } catch (InputMismatchException noGame) {
