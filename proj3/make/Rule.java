...
class Rule {
    ...
    private void checkFinishedDependencies() {
        for (int succ : _depends.successors(this.getVertex())) {
            if (_depends.getLabel(succ).isUnfinished()) {
                error("error it is uninished.");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        for (int succ : _depends.successors(_vertex)) {
            if (_time < _depends.getLabel(succ).getTime()) {
                return true;
            }
        }
        return false;
    ...
    void rebuild() {
        ...
        if (outOfDate()) {
            ...
            _time = new Integer(_maker.getCurrentTime());
        }
        _finished = true;
    }
